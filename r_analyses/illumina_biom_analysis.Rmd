...
superkingdom_plot <- superkingdom_absolute_abun %>% 
  ...
  filter(sum_type != 'total_sums')

superkingdom_plot

ggplot(data = superkingdom_plot) +
  geom_col(mapping = aes(x=sample_name, y = count, fill=sum_type), position = "fill")
...
ps_sample_data <- ps %>% sample_data()

sample_pipeline_filtered <- sample_pipeline_filtered %>% as_tibble %>%  left_join(.,as_tibble(ps_sample_data)$sample_sum, by = .experiment_type, copy = TRUE)

...
We by loading some useful R packages:
...
#### Formatting column names into snakecase
...
variables unchanged. It is convenient to standardize all variable names
to snake case; an easy way to do this is with the function
`janitor::clean_names()`.
...
To break down the structure of the resulting dataframe, the first column
contains the entire taxonomic structure of one organism in one variable,
like so:

`Bacteria;;Proteobacteria;Alphaproteobacteria;Rhizobiales;Xanthobacteraceae;Azorhizobium;Azorhizobium caulinodan`

The following columns

```{r}

view(as_tibble(tax[1:3, 1]))
...
I do not fully understand the graph below.
...
prior to doing some downstream analysis. There are various ways to go
about this, but some things one can do:
...
```{r}
ps %>% filter_sample_data(!sample_sum > 1e4) %>% sample_data %>% .[["experiment_type"]]
```

-   Subset to just viruses (the Rothman analysis also only considers
    viruses)
-   Remove species not appearing in at least 2 samples and 10 reads,
    which will speed up calculations and likely make our results more
    meaningful since these identifications can easily be spurious.
-   Aggregate to the genus level, since I don't currently trust CZID
    species assignments <!--  -->
...
Now there are only `r ntaxa(ps_plot)` species being considered, compared
to `r ntaxa(ps)` in the entire CZID output.

Taxa with a missing genus name have been filtered out; this is the
phyloseq default but it can have a big effect. In our case we only drop
13 taxa by dropping taxa with missing genus name.

```{r}
ps_plot_with_na_genus <- ps %>%
  filter_tax_table(superkingdom == 'Viruses') %>%
  filter_taxa2(~ sum(. > 0) > 2 & sum(.) >= 10) %>%
...
Note, the names of the taxonomic features after aggregation are set to
the name of the most abundant feature within the genus; they are not
automatically set to the genus name. That is easy to do manually,
provided that you have unique genus names (which is not always the
case). It can be useful to check if the genus names are unique for
plotting by genus later on: 
...
```{r}
nmds <- ps_plot %>%
    speedy_seq::transform_sample_counts(~ . / sum(.)) %>%
...
#Will have to make the creation of the experiment_class more robust in the future.
...
sam_plot <- sam_plot %>% sample_data %>% as_tibble %>%
  add_column(experiment_class = experiment_class)
```

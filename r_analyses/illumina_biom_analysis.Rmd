...
sample_pipeline_filtered <- sample_pipeline %>% 
  filter(total_reads > 1e4)

ps_sample_type_and_sum <- ps %>% sample_data() %>% as_tibble() %>% select(experiment_type, sample_sum)

sample_pipeline_filtered <- sample_pipeline_filtered %>% left_join(.,ps_sample_type_and_sum, by = c("Experiment.Type" = "experiment_type"), copy = TRUE)


```

```{r}
barplot_pipeline <- sample_pipeline_filtered %>% select(total_reads, reads_after_star, reads_after_trimmomatic, reads_after_priceseq, reads_after_czid_dedup, sample_sum,sample_name)
...
df <- data.frame(
  experiment_type = c("A", "A", "B", "B", "C", "C"),
  counts = c(10, 20, 30, 40, 50, 60)
)

ggplot(data = df, aes(x = counts)) +
  geom_histogram(bins = 10) +
  facet_wrap(~ experiment_type) +
  xlab("Counts at different experiment stages") +

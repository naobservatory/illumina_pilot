...
pipeline. For this we again subsample to larger samples (reads \>1000).
We can also add information about the experiment class and the total
number of assigned reads (`total_reads`)
...
ps_sample_type_and_sum <- ps %>% 
  sample_data() %>% 
  as_tibble() %>% 
  select(experiment_type, sample_sum)

sample_pipeline_subset <- sample_pipeline_subset %>% left_join(.,ps_sample_type_and_sum, by = c("Experiment.Type" = "experiment_type"), copy = TRUE)
...
```{r}
abun_table %>% select(experiment_type, sample_sum) %>% kable
```

...
First, we subset all unique viral species taxonomies from the kraken2 taxonomic reports.
...
kraken_viral_species <- kraken_taxonomies_expanded %>%
  ...
  # Dropping all rows with duplicate virus names. We are using keep_all to keep all other columns, not only virus.
...
Doing this we find that only **`r virus_or_genus_present`** of the **`r length(kraken_viral_species$species)`** species in the kraken2 match onto the ICTV table, either by species or genus name. This equates to **`r round((virus_or_genus_present / length(`r length(kraken_viral_species$species) *100))` %** of species in our kraken2 analyses.
...
Next, we want to identify on which taxonomic level we want link entries. We do this by figuring out which taxonomic level in the ICTV table is easily linkable while also maintaining unique genome composition.

As seen below there are more than 1400 species that do not have the same genome composition as other species in their genus. This means that we cannot link entries by genus, as this would result in a lot of misclassifications.
...
non_unique_composition_w_o_pos_neg <- composition_w_o_pos_neg %>%
  group_by(genus) %>%
  #Filter all genera with more than one genome composition
  filter(n_distinct(genome_composition) > 1) %>%
  glimpse
...
```{r}
taxonomy_reference %>%
  filter(genus == "Alphapleolipovirus") %>%
  glimpse
```

...
```{r}
kraken_viral_species_complete <- kraken_viral_species %>%
...
```{r}
desired_columns = c(
  'single_stranded_double_stranded',
  'dna_rna',
  'plus_minus',
  'rt_non_rt'
)
...
# Make the below vastly more efficient!
add_species_genome_comp <- function(sample_report_line) {
  if (species %in% taxonomy_reference_split_genome_comp$species) {
    #If the species is in the taxonomy reference, we add the desired_columns to the sample_report_line
    mutate(
        ...
        rt_non_rt = taxonomy_reference_split_genome_comp %>%
          filter(species == species) %>%
          pull(rt_non_rt)
  } else if (species %in% taxonomy_reference_split_genome_comp$virus_name_s) {
      mutate(
          single_stranded_double_stranded = taxonomy_reference_split_genome_comp %>%
            filter(virus_name_s == species) %>%
            pull(single_stranded_double_stranded),
          dna_rna = taxonomy_reference_split_genome_comp %>%
            filter(virus_name_s == species) %>%
            pull(dna_rna),
          ...
          rt_non_rt = taxonomy_reference_split_genome_comp %>%
            filter(virus_name_s == species) %>%
            pull(rt_non_rt)
  } else if (genus %in% taxonomy_reference_split_genome_comp$genus) {
      if n_unique_single_stranded_double_stranded <- taxonomy_reference_split_genome_comp %>%
          filter(genus == genus) %>%
          pull(single_stranded_double_stranded) %>% unique() %>% length() == 1 {
            mutate(
      ...
      if n_unique_dna_rna <- taxonomy_reference_split_genome_comp %>%
          filter(genus == genus) %>%
          pull(dna_rna) %>% unique() %>% length() == 1 {
            mutate(
              dna_rna = dna_rna)
          } else {
            mutate(
      ...
      if n_unique_plus_minus <- taxonomy_reference_split_genome_comp %>%
          filter(genus == genus) %>%
          pull(plus_minus) %>% unique() %>% length() == 1 {
            mutate(
      ...
      if n_unique_rt_non_rt <- taxonomy_reference_split_genome_comp %>%
          filter(genus == genus) %>%
          pull(rt_non_rt) %>% unique() %>% length() == 1 {
            mutate(
              rt_non_rt = rt_non_rt)
          } else {
            mutate(

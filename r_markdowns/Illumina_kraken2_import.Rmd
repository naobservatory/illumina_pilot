...
kraken_viral_species <- kraken_taxonomies_expanded %>%
  ...
  # Dropping all rows with duplicate virus names, thus only keeping  We are using keep_all to keep all other columns, not only virus.
...
taxonomy_reference_split_genome_comp %>%
  #Print number of occurences for each of unique genome composition
  count(DNA_RNA),
  count(single_stranded_double_stranded),
  print
```
...
kraken_viral_species_complete <- kraken_viral_species %>%
  select(!full_taxonomy) %>%
    left_join(
      ...
      by = c('species' = 'scientific_name') #Joining on species name
    )
```
